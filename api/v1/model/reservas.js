...
const createReservaFromOperaciones = async (body) => {
  try {
    ...
    const params_servicio = [id_servicio, solicitud.total, parseFloat((solicitud.total * .84).toFixed(2)), parseFloat((solicitud.total * .16).toFixed(2)), true, null, null];

    const response = await executeTransaction(query_servicio, params_servicio, async (results, connection) => {
      try {
        ...
        const { confirmation_code, id_viajero, hotel, check_in, check_out, room, total, status, id_usuario_generador } = solicitud;
        const params_solicitud = [id_solicitud, id_servicio, id_usuario_generador, confirmation_code, id_viajero, hotel, check_in, check_out, room, total, status];
        const response_solicitudes = await connection.execute(query_solicitudes, params_solicitud);

        const id_booking = `boo-${uuidv4()}`
        const { estado, nombre_hotel, subtotal, impuestos, tipo_cuarto, noches, costo_subtotal, costo_total, costo_impuestos, codigo_reservacion_hotel, items } = body
        ...
        const params_items = itemsConId.flatMap(item => [item.id_item, null, null, item.total, item.subtotal, item.impuestos, null, (new Date()).toISOString().split("T")[0], id_hospedaje, item.costo_total, item.costo_subtotal, item.costo_impuestos, item.costo_iva, item.total]);
        ...
        const params_pagos = [id_pago_credito, id_servicio, solicitud.total, id_usuario_generador, null, (new Date()).toISOString().split("T")[0], 0, true, solicitud.total, parseFloat((solicitud.total * .84).toFixed(2)), parseFloat((solicitud.total * .16).toFixed(2))]
        const response_pagos = await connection.execute(query_pagos, params_pagos);
      ...
      `;
        ...
        const params_items_pagos = itemsConId.flatMap(item => [
          item.id_item, id_pago_credito, item.total
        ]);
